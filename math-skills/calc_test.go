...
func TestOutput(t *testing.T) {
	...
	// docker command: "./stat-bin-dockerized/stat-bin/run.sh", "math-skills"
	binaryOutput, err := runCommand("./math-skills")
	if err != nil {
		t.Fatalf(Red+"Error running binary executable: %v"+Reset, err)
	}

	// Run the Python program.
	pythonOutput, err := runCommand("python3", "calc.py", "data.txt")
	if err != nil {
