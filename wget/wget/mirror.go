...
func MirrorWebsite(wg *sync.WaitGroup) {

	var log io.Writer
	// Parameters
	targetURL := URLs[0]
	rejectList := RejectedSuffixes // Reject image files
	excludeList := ExcludedPaths   // Exclude private directories
	convertLinks := ConvertLinks   // Convert links to local resources
...
func processNode(n *html.Node, baseURL, outputDir string, rejectList, excludeList []string, convertLinks bool, log io.Writer, wg *sync.WaitGroup) {
...
func handleResource(attrValue, baseURL, outputDir string, rejectList, excludeList []string, convertLinks bool, log io.Writer, wg *sync.WaitGroup) string {
...
func processCSSContent(cssContent, baseURL, outputDir string, rejectList, excludeList []string, convertLinks bool, log io.Writer, wg *sync.WaitGroup) string {
...
func downloadResource(resourceURL, outputPath string, log io.Writer) error {
